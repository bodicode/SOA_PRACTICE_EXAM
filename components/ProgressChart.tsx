            ...
            <div className="bg-white p-3 border border-gray-100 shadow-md rounded-lg text-sm">
                ...
                <div className="text-blue-600 font-bold">
                    ...
                    <span className="text-xs font-normal text-gray-500 ml-1">
                        (Đúng {payload[0].payload.score}/{payload[0].payload.total} câu) - {payload[0].payload.mode}
                    </span>
                </div>
            </div>
        )
...
export default function ProgressChart({ data }: ProgressChartProps) {
    if (!data || data.length === 0) {
        return (
            <Card className="h-full flex items-center justify-center bg-gray-50 border-dashed border-none shadow-none">
